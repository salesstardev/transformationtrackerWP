...
$curl = curl_init();
    ...
    foreach ($responseArray as $tracker)
    {
        ...
        {   
            ...
            $linkID = $tracker->{'_id'};
            $viewLink = "https://app.salesstar.com/view-tracker/?{$linkID}";
            $editLink = "https://app.salesstar.com/edit-tracker/?{$linkID}";
    
            if (stripos($companyName, $searchTerm) !== false || stripos($trackerName, $searchTerm) !== false || stripos($practice, $searchTerm) !== false || stripos($coach, $searchTerm) !== false)
            {
